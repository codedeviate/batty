...
- **Within-block markdown source precision.** The `m`-toggle now preserves scroll position via a block-granular source ↔ rendered map (per-block rendering with pulldown_cmark + termimad). A wrapped 6-row paragraph maps all rendered rows to the paragraph's source-line start — coarse but useful. Bumping to per-row precision would require replacing termimad with a custom renderer.
- **Per-row markdown gutter** displaying source-line numbers next to rendered output. The interactive status bar shows `rendered N/M ↔ src K` for orientation; a full per-row gutter column would need a row-by-row source-line resolver in the render frame.
