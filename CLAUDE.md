...
- **No dead code in main:** if a method isn't used, either remove it or `#[allow(dead_code)]` with a comment explaining why it stays. Three pre-existing dead-code warnings are tolerated (`Cli::parse_from_args`, `StyleFlags::any`, `PrinterConfig::wrap`) — don't add more without justification.
